...
export class AppService {
  ...
  public versionCntURL: string = "http://localhost:3000/api/vc";
  public versionsURL: string = "http://localhost:3000/api/versions";
  public targetCompareDoc: string = "http://localhost:3000/api/tcd";
  ...
  public getDocsCount(): Promise<any> {
    return this._http.get(this.countURL).toPromise().then(this.extractData).catch(this.handleError);
  }

  public getDoc(id: number): Promise<any> {
    let url = this.getDocURL + "/" + id;
    return this._http.get(url).toPromise().then(this.extractData).catch(this.handleError);
  }

  public getVersionCount(impName: string): Promise<any> {
    let url = this.versionCntURL + '/' + impName;
    return this._http.get(url).toPromise().then(this.extractData).catch(this.handleError);
  }

  public getVersions(impName: string): Promise<any> {
    let url = this.versionsURL + '/' + impName;
    return this._http.get(url).toPromise().then(this.extractData).catch(this.handleError);
  }
  ...
  public getTargetDoc(name: string, version: string, stname: string) {
    var url = this.targetCompareDoc + '/' + name + '/' + version + '/' + stname;
    return this._http.get(url).toPromise().then(this.extractData).catch(this.handleError);
  }
